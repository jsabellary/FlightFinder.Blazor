...
namespace FlightFinder.Blazor.Services
{
    public class AppState
    {
        ...
        public async Task Search(SearchCriteria criteria)
        {
            ...
            try
            {
                ...
                SearchResults = await response.Content.ReadFromJsonAsync<Itinerary[]>() ?? Array.Empty<Itinerary>();
            }
            catch (Exception)
            {
                // API not available - set empty results
